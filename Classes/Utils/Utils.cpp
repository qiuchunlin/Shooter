...
Vec2 Utils::getPosInMap(Vec2 pos, TMXTiledMap* pMap)
{
	...
	int y = ((pMap->getMapSize().height * pMap->getTileSize().height) - pos.y) / pMap->getTileSize().height;
	return Point(x, y);
}

float Utils::getPosAngle(Vec2 targetPos, Vec2 curPos)
{
	float fX = targetPos.x - curPos.x;
	float fY = targetPos.y - curPos.y;
	float fAngle = 0;
	if (fX < 0 && fY > 0)
	{
