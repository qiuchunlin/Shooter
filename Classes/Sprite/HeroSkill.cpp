...
void  HeroSkill::skillEffect()
{
	if (_nSkillId == 1)
	{
        ...
        this->runAction(Sequence::create(DelayTime::create(1.0f),fireAction,DelayTime::create(3.5f),releaseAction,NULL));
	}
    else if (_nSkillId == 2)
    {
		...
		GameService::getInstance()->getGameScene()->getHero()->addChild(_pSkillArmature);
		LayoutUtil::layoutParentCenter(_pSkillArmature, 0, 15);
